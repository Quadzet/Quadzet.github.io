...
self.addEventListener('message', function(e) {
    ...
    function handleEvent(event, eventList, futureEvents)
    {
        ...
        if(event.type == "combatStart") {
            ...
            let source = Actors[event.source];
            source.onGCD = false
            performAction(event.timestamp, source, eventList, futureEvents)
        }
            ...
            if(event.auraType == "debuff")
                Actors[event.owner].debuffs[event.name].expire(event.timestamp, eventList, futureEvents)
            else if(event.auraType == "buff")
                Actors[event.owner].buffs[event.name].expire(event.timestamp, eventList, futureEvents)
        }
