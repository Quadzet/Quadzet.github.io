class Aura {
    constructor(input) {
        ...
        if (!input.owner) this.owner = "unknown"; else this.owner = input.owner;
    ...
    apply(timestamp, owner, source, eventList, futureEvents) {
        if(this.type == "debuff")
            Actors[owner].debuffs[this.name] = this
        else 
            Actors[owner].buffs[this.name] = this
        ...
        Actors[owner].armor += this.armorMod
        Actors[owner].block += this.blockMod
        ...
        let index = futureEvents.findIndex(e => {return (e.type == "auraExpire" && e.name == this.name && e.owner == this.owner)})
        if(index >= 0)
    ...
    removeStack(timestamp, owner, eventList, futureEvents) {
        if(this.stacks == 1)
            this.expire(timestamp, eventList, futureEvents)
        else {
            eventList.push({
                type: "auraExpire",
                name: this.name,
