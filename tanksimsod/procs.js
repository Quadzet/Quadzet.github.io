...
class WildStrikesProc extends Proc {
    ...
    handleEvent(source, target, event, reactiveEvents, futureEvents) {
        ...
        if (event.type == "damage" && event.source == source.name && ["MH Swing", "Bloodthirst", "Raging Blow", "Quickstrike", "Devastate", "Sunder Armor", "Heroic Strike", "Revenge"].includes(event.name) && landedHits.includes(event.hit) && event.timestamp >= this.ICDTimer) {
            let rng = Math.random()
            if (rng < 0.2) {
                let procEvent = {
                    "type": "extra attack",
