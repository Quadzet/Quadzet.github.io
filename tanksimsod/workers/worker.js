...
self.addEventListener('message', function(e) {
    ...
    Actors = {
        "Tank": new Actor("Tank", globals.tankStats, TankAbilities(globals.tankStats), TankProcs),
        ...
        eventList.forEach(event => {
            if (event) {
                if (event.source == "Boss" && event.type == "damage") {
                    ...
                    if (event.threat && event.name) {
                        if(!results.breakdown[`${event.name}`]) results.breakdown[`${event.name}`] = []
                        if(results.breakdown[`${event.name}`][i]) results.breakdown[`${event.name}`][i] += event.threat/globals.config.simDuration
...
/*
    ...
    if(globals.tankStats.talents.shieldslam && !globals.tankStats.dualWield)
        playerAbilities.push(new ShieldSlam("Shield Slam", 6000, 20, true));
    else if (globals.tankStats.talents.bloodthirst)
        playerAbilities.push(new Bloodthirst("Bloodthirst", 6000, 30, true));

    playerAbilities.push(new Revenge("Revenge", 5000, 5, true))
    playerAbilities.push(new HeroicStrike("Heroic Strike", 0, 15 - globals.tankStats.talents.impHS, false))
    playerAbilities.push(new SunderArmor("Sunder Armor", 0, 15 - globals.tankStats.talents.impSA, true))
