...
function get_available_talents(selected_talents) {
...
var n_combs = 0;
var tree = { };
function get_n_talent_combinations(selected_talents, new_talent, depth) {
  ...
  if (tree[new_talent] > 0) {
    tree[new_talent] = 1;
  } else {
    tree[new_talent]++;
  }

  var new_talents = get_available_talents(seleccted_talents);

  for (new_talent in new_talents) {
    get_n_talent_combinations(selected_talents, new_talent, depth);


  // Select the new talent
  // Find which talents are now accessible
  // Recurse over the list of accessible talents
  //
